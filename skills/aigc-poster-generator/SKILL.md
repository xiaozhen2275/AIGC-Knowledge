---
name: aigc-poster-generator
description: 根据海报设计简报生成或准备 AIGC 课程、工作坊、读书会海报。必须在用户要求生成海报、封面图、商品主图、介绍图、产品介绍长图、图片生成提示词、Image2 图片、即梦/可灵提示词，或要求基于 10_海报设计简报.md 等设计简报批量产出视觉图时使用。
---

# AIGC 海报生成器

使用本 skill 时，必须按阶段生成海报资产，不要一次性生成所有图片。同一项目内的所有图片必须保持统一的视觉系统。

## 触发条件

出现以下任一情况，必须使用本 skill：

- 用户说“生成海报”“做海报”“出图”“生成封面图”“生成主图”“生成长图”。
- 用户要求基于 `10_海报设计简报.md` 或其他海报设计简报生成视觉图。
- 用户要求生成 `三、3 张封面图（1080x1080）`。
- 用户要求生成 `四、商品主图/介绍图`。
- 用户要求生成 `五、产品介绍长图`。
- 用户要求调用 Codex 的图片生成能力、Image2 或其他图片生成工具来生成课程/活动/读书会海报。
- 用户要求输出即梦、可灵或其他图片大模型可复制使用的海报提示词。
- 用户要求根据已有海报风格、配色方案、参考图、模板，批量生成一组统一风格的活动宣传图。

出现以下情况，可以使用本 skill：

- 用户只想优化海报文案，但该文案属于封面图、商品主图、介绍图或长图的一部分。
- 用户想把设计简报拆成分图提示词、图片生成清单或后期文字叠加清单。
- 用户想检查一组海报是否风格统一、配色统一、生成顺序合理。

出现以下情况，不要使用本 skill：

- 用户只是在修改普通课程文档、主持稿、报名表或运营清单，且不涉及海报或图片生成。
- 用户只是在讨论读书会产品定位、课程内容设计或 WorkBuddy 提示词。
- 用户明确要求不要生成图片，也不要生成图片模型提示词。

## 必读上下文

开始海报工作前，先读取：

1. `AGENTS.md`
2. `00_AGENT初始化总纲.md`
3. `10_海报设计简报.md`

如果用户指定了其他海报设计简报，则读取用户指定的设计简报。

## 工作流闸门

严格按以下顺序执行：

1. 理解并确认整体设计方向。
2. 只生成 `三、3 张封面图（1080x1080）`。
3. 用户确认封面图效果后，再生成 `四、商品主图/介绍图` 的全部图片。
4. 商品主图/介绍图完成后，再生成 `五、产品介绍长图`。

不要跳过封面图预览闸门。如果 3 张封面图未达到预期，回到第 1 步，整体调整设计风格、配色方案、版式逻辑和是否包含人物的策略后再生成。

## 第 1 步：确认整体风格

读取设计简报，提取：

- 设计思路
- 配色方案
- 字体和版式气质
- 重复视觉元素
- 海报章节和所需图片数量
- 禁止事项和风格边界

如果设计简报已经包含设计风格和配色方案，先询问：

```text
是否按设计文档现有风格执行？
```

同时必须主动询问：

```text
是否需要包含人物海报？
```

如果用户要调整风格、配色或参考图，询问是否提供：

- 海报模板
- 参考图片或截图
- 品牌色
- 风格关键词
- 喜欢或不喜欢的设计案例

全局规则：任何风格、配色、人物、版式或参考图调整，都必须应用到整套海报。不要让每一张图各用一套风格。

## 第 2 步：生成 3 张封面图

只生成 `三、3 张封面图（1080x1080）`。

要求：

- 尺寸：`1080x1080`
- 保持统一的配色系统、视觉语言和版式家族
- 保留每张封面自己的主标题、副标题、信息条、行动号召和构图目标
- 使用图片模型时，不依赖模型生成密集小字；优先生成清晰的大标题和主视觉，细节文案可后期叠加

生成后停止，并询问用户确认：

```text
这 3 张封面图的整体风格、配色、人物策略和版式方向是否通过？
```

如果用户未确认，不要继续生成商品主图/介绍图。

## 第 3 步：生成商品主图/介绍图

只有封面图通过后，才生成 `四、商品主图/介绍图` 的全部图片。

要求：

- 沿用已经确认的整体风格
- 按设计简报中的实际数量全部生成，不擅自新增或遗漏
- 标题层级、卡片节奏、背景质感、强调色和图标风格保持一致
- 每张图只服务自己的功能：路径、体验、方法、成果、知识资产、报名须知等

生成后，简要说明生成了哪些图片，以及哪些文字建议后期精确叠加。

## 第 4 步：生成产品介绍长图

生成 `五、产品介绍长图` 前，必须确认以下可变活动信息：

- 价格
- 日期和时间
- 场地名称
- 完整地址
- 二维码图片或二维码内容
- 报名/咨询方式

如有任何信息缺失，先向用户询问。不要默认沿用设计简报里的旧活动信息，除非用户明确确认。

要求：

- 沿用封面图阶段确认的整体风格
- 保留设计简报中的长图屏幕顺序
- 二维码区域必须干净、可扫码
- 行动号召文案要短，适合手机端阅读
- 使用图片模型时，不依赖模型生成密集中文；必要时单独提供精确叠加文字

## 生成模式

### Codex 且具备图片生成能力

如果 Codex 当前可调用图片生成工具，直接按阶段生成图片。每张图或每个同类批次单独写提示词。不要在封面图阶段混入后续商品主图或长图。

### 其他 Agent 且具备图片生成 API

如果其他 Agent 具备图片生成 API，输出结构化图片提示词，包含：

- 图片名称
- 尺寸
- 生成目标
- 视觉布局
- 配色方案
- 必须出现的文字
- 负向约束
- 后期文字叠加说明

### 其他 Agent 且没有图片生成 API

如果没有图片生成 API，输出可复制到即梦、可灵或类似图片模型中的提示词，包含：

- 全局风格提示词
- 每张海报的独立提示词
- 负向提示词
- 精确文字叠加清单
- 生成顺序和确认闸门

## 质量检查

输出前检查：

- 当前阶段是否符合工作流闸门
- 所有图片是否共享同一套视觉系统
- 风格变化是否已经整体应用
- 封面图是否已确认后才进入商品主图
- 商品主图/介绍图是否完成后才进入长图
- 长图的价格、时间、地点、二维码和报名方式是否已确认
- 输出是否仍然服务低压力、2 小时、现场带走可见成果的工作坊定位
